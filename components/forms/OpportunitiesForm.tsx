...
export function OpportunitiesForm() {
  ...
  const mapPreferredWorkingToInt = (value: string) => {
    switch (value) {
      case "remote":
        return 0;
      case "onsite":
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      console.debug("Opportunity FormData entries:", entries);
      ...
      mapField("hearUs", "HearUs");
      mapField("coverLetter", "CoverLetter");

      // Map preferredWorking to PreferredWorking integer
      fd.set("PreferredWorking", String(mapPreferredWorkingToInt(preferredWorking)));
      ...
      const res = await axios.post("/api/opportunity", fd, {
        // Let browser set Content-Type with boundary
        headers: { Accept: "application/json" },
      });

      if (res.status >= 200 && res.status < 300) {
        Swal.fire({ icon: "success", title: "Submitted", text: "Your application was submitted successfully.", timer: 2500, showConfirmButton: false });
        form.reset();
        setPreferredWorking("");
        setHeardAboutUs("");
        setFileName("");
    ...
    <form className="space-y-6" onSubmit={handleSubmit}>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
        ...
        <label className="flex flex-col gap-2 text-sm">
          ...
          <select
            ...
            className="rounded-lg border px-4 py-2 text-sm text-slate-900 transition focus:border-[#17aac0] focus:outline-none focus:ring-2 focus:ring-[#c6eff3]"
          ...
          >
            ...
            <option value="events">Events or Workshops</option>
            <option value="website">Our Website</option>
            <option value="partners">Partners or Affiliates</option>
      ...
      <div className="space-y-3">
        ...
        <div className="flex flex-wrap gap-4">
          {["Remote", "OnSite", "Hybrid"].map((category) => (
            <label
              key={category}
            ...
            >
              <input
                type="radio"
                name="preferredWorking"
                value={category.toLowerCase()}
                checked={preferredWorking === category.toLowerCase()}
                onChange={(e) => setPreferredWorking(e.target.value)}
                className="w-4 h-4 text-[#17aac0] focus:ring-[#17aac0]"
              />
      ...
      <div className="space-y-2">
        <label className="block text-sm font-medium text-slate-700">
          Upload Your Resume <span className="text-slate-400">(Optional)</span>
        </label>
