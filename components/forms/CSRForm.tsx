...
export function CSRForm() {
  ...
  const mapCategoryToInt = (value: string) => {
    switch (value) {
      case "student":
        return 0;
      case "professional":
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      console.debug("CSR FormData entries:", entries);
      ...
      mapField("description", "IdeaOfEvent");

      // Map requesterCategory to RequestCategory integer
      fd.set("RequestCategory", String(mapCategoryToInt(requesterCategory)));
      ...
      const res = await axios.post("/api/csr", fd, {
    ...
    <form className="space-y-6" onSubmit={handleSubmit}>
      ...
      <div className="space-y-3">
        ...
        <div className="flex flex-wrap gap-4">
          {["Student", "Professional", "Institute"].map((category) => (
            <label
              key={category}
            ...
            >
              <input
                ...
                checked={requesterCategory === category.toLowerCase()}
                onChange={(e) => setRequesterCategory(e.target.value)}
      ...
      <TextArea
        label="Idea or Event Description"
        name="description"
        rows={8}
        placeholder="Have an idea for a CSR event? Tell us here!"
        required
      />
